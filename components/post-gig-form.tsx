  ...
  return (
    <form onSubmit={submit} className="space-y-5 pb-5">
      ...
      <section className="space-y-5 rounded-[1.75rem] bg-white p-5 shadow-[0_14px_34px_rgba(53,32,79,0.08)] sm:p-7">
        ...
        <label className="block space-y-2"><span className={labelClass}>Gig name</span><input className={inputClass} placeholder="e.g. Hangout with Slice" required /></label>
        <label className="block space-y-2"><span className={labelClass}>Tell people about it</span><textarea className={`${inputClass} min-h-32 resize-none py-3`} placeholder="What are you bringing people together to do?" required /></label>
        <div className="grid gap-4 sm:grid-cols-2"><label className="block space-y-2"><span className={labelClass}>Category</span><select className={inputClass} defaultValue="events"><option value="events">Events</option><option value="community">Community</option><option value="hospitality">Hospitality</option><option value="projects">Projects</option></select></label><label className="block space-y-2"><span className={labelClass}>Gig type</span><select className={inputClass} defaultValue="in-person"><option value="in-person">In-person</option><option value="remote">Remote</option></select></label></div>
      </section>
      ...
      <section className="space-y-5 rounded-[1.75rem] bg-white p-5 shadow-[0_14px_34px_rgba(53,32,79,0.08)] sm:p-7">
        ...
        <label className="block space-y-2"><span className={labelClass}>Place</span><div className="relative"><span className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-purple-gray">⌖</span><input className={`${inputClass} pl-10`} placeholder="Search for a place or add an address" required /></div></label>
        <div className="grid gap-4 sm:grid-cols-2"><label className="block space-y-2"><span className={labelClass}>Date</span><input type="date" className={inputClass} required /></label><label className="block space-y-2"><span className={labelClass}>Start time</span><input type="time" className={inputClass} required /></label></div>
      </section>
      ...
      <section className="space-y-5 rounded-[1.75rem] bg-white p-5 shadow-[0_14px_34px_rgba(53,32,79,0.08)] sm:p-7">
        ...
        <label className="block space-y-2"><span className={labelClass}>Total slots</span><input type="number" min="1" defaultValue="5" className={inputClass} required /></label>
      ...
      <section className="space-y-5 rounded-[1.75rem] bg-white p-5 shadow-[0_14px_34px_rgba(53,32,79,0.08)] sm:p-7">
        ...
        {kind === "volunteer" ? <label className="block space-y-2"><span className={labelClass}>Incentive or support</span><input className={inputClass} placeholder="e.g. Transport support and breakfast provided" /></label> : <label className="block space-y-2"><span className={labelClass}>Payment</span><input className={inputClass} placeholder="e.g. ₦35,000 per person" /></label>}
        <label className="block space-y-2"><span className={labelClass}>Instructions</span><textarea className={`${inputClass} min-h-28 resize-none py-3`} placeholder="Dress code, arrival time, what to bring..." /></label>
      ...
      {submitted ? <div role="status" className="rounded-2xl bg-success/15 p-4 text-sm font-semibold text-[#328e1c]">Your gig draft is ready. Publishing will connect once the backend is wired.</div> : null}
      <button type="submit" className="flex min-h-14 w-full items-center justify-center rounded-2xl bg-black px-5 text-base font-bold text-white shadow-[0_14px_28px_rgba(0,0,0,0.16)] transition-colors hover:bg-purple">{kind === "volunteer" ? "Create volunteer gig" : "Create paid gig"}</button>
      <p className="text-center text-xs leading-5 text-purple-gray">You can edit the details before publishing. Offer will never publish without your confirmation.</p>
    </form>
  );
}
