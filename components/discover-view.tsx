    ...
    <div className="space-y-8">
      ...
      {liveEmpty ? (
        <div className="rounded-[1.5rem] border border-dashed border-black/15 bg-white px-5 py-12 text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-purple-gray">You&apos;re looking at live Offer data, not the demo catalog. Be the first to bring people together — volunteer gigs first.</p>
          <Link href="/post" className="mt-5 inline-flex min-h-11 items-center rounded-full bg-black px-5 text-sm font-bold text-white">Post a gig</Link>
        </div>
      ) : null}
      {!liveEmpty && topical.length > 0 ? <section className="space-y-4" aria-labelledby="topical-heading"><div className="flex items-center justify-between"><h2 id="topical-heading" className="text-xl font-bold tracking-[-0.04em]">Topical</h2><span className="text-sm font-semibold text-purple-gray">{topical.length} available</span></div><div className="grid gap-5 md:grid-cols-2">{topical.map((gig) => <GigCard key={gig.slug} href={`/gigs/${gig.slug}`} {...gig} />)}</div></section> : null}
      {!liveEmpty && paid.length > 0 ? <section className="space-y-4" aria-labelledby="for-you-heading"><div className="flex items-center justify-between"><h2 id="for-you-heading" className="text-xl font-bold tracking-[-0.04em]">For you</h2><span className="text-sm font-semibold text-purple-gray">{paid.length} available</span></div><div className="grid gap-5 md:grid-cols-2">{paid.map((gig) => <GigCard key={gig.slug} href={`/gigs/${gig.slug}`} {...gig} />)}</div></section> : null}
      {!liveEmpty && filtered.length === 0 ? <div className="rounded-[1.5rem] border border-dashed border-black/15 bg-white px-5 py-12 text-center"><p className="text-base font-bold">No gigs match that search</p><p className="mt-2 text-sm leading-6 text-purple-gray">Try another place, category, or gig type.</p><button type="button" onClick={() => { setQuery(""); setCategory("All gigs"); setKind("All"); }} className="mt-5 min-h-11 rounded-full bg-black px-5 text-sm font-bold text-white">Clear filters</button></div> : null}
