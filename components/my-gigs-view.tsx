...
function MyGigCard({ gig }: { gig: MyGig }) {
  return (
    <Link href={`/gigs/${gig.slug}`} className="group flex gap-3 rounded-[1.5rem] bg-white p-3 shadow-[0_12px_28px_rgba(53,32,79,0.07)] transition-transform hover:-translate-y-0.5 sm:gap-4 sm:p-4">
      ...
      <div className="min-w-0 flex-1 py-1">
        ...
        <span className={`mt-3 inline-flex rounded-full px-2.5 py-1 text-[10px] font-bold ${gig.status === "Upcoming" ? "bg-purple/10 text-purple" : "bg-success/15 text-[#37951a]"}`}>{gig.status}</span>
      </div>
    </Link>
  );
}
...
export function MyGigsView() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-7">
      ...
      <div className="grid grid-cols-3 gap-3"><Stat value="3" label="Joined" /><Stat value="1" label="Hosted" /><Stat value="12" label="People met" /></div>
